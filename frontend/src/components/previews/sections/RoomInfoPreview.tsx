    ...
    <div>
      ...
      <div className="text-sm text-muted-foreground space-y-1.5">
        ...
        {roomInfo.pet_policy?.pets_allowed !== undefined && (
          <p><span className="text-foreground font-medium">Pets Allowed:</span> {roomInfo.pet_policy.pets_allowed ? 'Yes' : 'No'}</p>
        )}
        {roomInfo?.internetAvailable !== undefined && (
          <p><span className="text-foreground font-medium">Internet Available:</span> {roomInfo.internetAvailable ? 'Yes' : 'No'}</p>
        )}
        {roomInfo?.airConditioning !== undefined && (
          <p><span className="text-foreground font-medium">Air Conditioning:</span> {roomInfo.airConditioning ? 'Yes' : 'No'}</p>
        )}
